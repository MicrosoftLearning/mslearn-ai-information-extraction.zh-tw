...
1. 在 Azure 入口網站中，瀏覽至您的 Azure AI 搜尋服務資源。 然後，在其 [概觀]**** 頁面上，選取 [匯入資料]****。
    ...
    ![Azure 搜尋服務中對匯入資料加上醒目提示的螢幕擷取畫面。](./media/overview-panel.png)
    
    > **注意：** 在 Azure AI 搜尋服務資源的 [**概觀**] 頁面中，工具列提供兩個選項：  
    > - **匯入資料** (傳統體驗)  
    > - **匯入資料 (新)** (新體驗)  
    >  
    > 此外，[**概觀**] 區段下方的 [開始] 面板包含可將您重新導向至**新 UI** 的 [**匯入**] 按鈕。  
    >  
    > 本課程中的指示參考**傳統匯入資料工作流程**。 為避免混淆，請務必從工具列中選取 [**匯入資料**]。
